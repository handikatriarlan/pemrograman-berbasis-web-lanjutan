...
function buildGameCanvas(){
	...
	numberTitleTxt.text = textDisplay.numberTextDisplay;
	...
	for(var n=0; n<score_arr.length+extraBall;n++){
		...
		if(gameSettings.bonusBall){			
			if(n == 0){
				totalNum--;	
				textString = textDisplay.matchTextDisplay.replace('[NUMBER]', totalNum);
				scoreString = textDisplay.scoreTextDisplay.replace('[NUMBER]', addCommas(score_arr[totalNum-1].prize));
				scoreNum = score_arr[totalNum-1].prize;
			}else if(n == 1){
				textString = textDisplay.bonusTextDisplay.replace('[NUMBER]', totalNum);
				scoreString = textDisplay.scoreTextDisplay.replace('[NUMBER]', addCommas(bonusScore[0].prize));
